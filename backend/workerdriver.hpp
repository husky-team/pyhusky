...
class WorkerDriver {
   ...
   private:
    ...
    static std::unordered_map<std::string, std::function<bool(WorkerDriverInfo&)>> worker_instr_handler_map;

    static std::unordered_map<
        std::string, std::function<void(const Operation& op, PythonSocket& python_socket, ITCWorker& daemon_socket)>>
        worker_operator_handler_map;
};
