...
protected:
    // thread handlers
    static void LogisticR_load_pyhlist_handler(PythonSocket & python_socket,
            ITCWorker & daemon_socket);

    // cpp handlers
    static void LogisticR_init_handler(const Operation & op,
            PythonSocket & python_socket,
            ITCWorker & daemon_socket);
    static void LogisticR_load_hdfs_handler(const Operation & op,
            PythonSocket & python_socket,
            ITCWorker & daemon_socket);
    static void LogisticR_train_handler(const Operation & op,
            PythonSocket & python_socket,
            ITCWorker & daemon_socket);
...
void Logistic_create_model_from_url(std::string name, std::string url, husky::lib::ml::DataFormat data_format) {
    ...
    local_SGD_LogisticR_model[name] = std::make_shared<LogisticModel<husky::lib::ml::LogisticRegression<double, double, is_sparse, ParameterBucket<double>>>>(new husky::lib::ml::LogisticRegression<double, double, is_sparse, ParameterBucket<double>>(num_features));
    std::static_pointer_cast<LogisticModel<husky::lib::ml::LogisticRegression<double, double, is_sparse, ParameterBucket<double>>>>(local_SGD_LogisticR_model.at(name))->get_model()->report_per_round = true;
}
...
void Logistic_train_model(std::string name, double alpha, int num_iter) {
    ...
    std::static_pointer_cast<LogisticModel<husky::lib::ml::LogisticRegression<double, double, is_sparse, ParameterBucket<double>>>>(local_SGD_LogisticR_model.at(name))->get_model()->template train<husky::lib::ml::FGD>(train_list, num_iter, alpha);
}

template <bool is_sparse>
BinStream Logistic_get_params(std::string name) {
    int n_param = std::static_pointer_cast<LogisticModel<husky::lib::ml::LogisticRegression<double, double, is_sparse, ParameterBucket<double>>>>(local_SGD_LogisticR_model.at(name))->get_model()->get_num_param();
    BinStream result;
    result << n_param;
    assert(n_param > 0);
    auto params_vec = std::static_pointer_cast<LogisticModel<husky::lib::ml::LogisticRegression<double, double, is_sparse, ParameterBucket<double>>>>(local_SGD_LogisticR_model.at(name))->get_model()->get_param();
