...
void PythonSocket::init(int wid) {
    ...
    pipe_to_python->connect(("ipc://pyhusky-session-"+master_port+"-proc-"+proc_id+"-"+std::to_string(wid)).c_str());

    pipe_from_python = new zmq::socket_t(*(Context::get_zmq_context()), ZMQ_PULL);
    pipe_from_python->bind(("ipc://cpphusky-session-"+master_port+"-proc-"+proc_id+"-"+std::to_string(wid)).c_str());
...
std::unordered_map<std::string,
    std::function<void(PythonSocket& python_socket, ITCWorker& daemon_socket)>> PythonConnector::handler_map;

PythonConnector::PythonConnector(ITCWorker & d_util, int wid):daemon_socket(d_util) {
    python_socket.init(wid);
}
...
void PythonConnector::start_python_proc(int lid, int gid, int num_workers) {
    std::string master_port = std::to_string(Context::get_config().get_master_port());
    std::string proc_id = std::to_string(Context::get_worker_info().get_process_id());
    extern_py_proc = popen(("python -m backend.python.python_backend " + std::to_string(lid)
    ...
    // python profile
    // extern_py_proc = popen(("python -m cProfile -o /tmp/prof-"+master_port+"-"+std::to_string(lid)+" backend/python/python_backend.py " + std::to_string(lid)
    //                        + " " + std::to_string(gid) + " " + proc_id + " " + std::to_string(num_workers) + " "
    //                        + master_port + " > /tmp/log-pyhusky-" + master_port + "-proc-" + proc_id + "-" + std::to_string(lid)).c_str() , "r");
     // N2N
...
void PythonConnector::send_string(const std::string& data) {
    zmq_send_string(python_socket.pipe_to_python, data);
}
void PythonConnector::send_binstream(BinStream& data) {
    zmq_send_binstream(python_socket.pipe_to_python, data);
}
}  // namespace husky
